...
export class GigService {
    ...
    public async createGig(
        ...
        try {
            if(!gig.title || !gig.description_md || !gig.freelancer_id) {
                throw new AppError('Title, description and freelancer ID are required', 400, 'TITLE_DESCRIPTION_FREELANCER_ID_REQUIRED');
            }
    ...
    public async getGigsByFreelancerId(freelancer_id: string): Promise<gigs[]> {
        try {
            ...
            throw new AppError('Error getting gigs by freelancer id', 500, 'DB_GIGS_GET_BY_FREELANCER_ID_FAILED');
        }
    }
