...
export class JobBidService {
    ...
    public async createJobBid(jobBid: Prisma.job_bidsUncheckedCreateInput): Promise<job_bids> {
        try {
            if(!jobBid.job_id || !jobBid.freelancer_id) {
                throw new AppError('Job ID and freelancer ID are required', 400, 'JOB_ID_FREELANCER_ID_REQUIRED');
            }
    ...
    public async updateJobBid(id: string, jobBid: Prisma.job_bidsUncheckedUpdateInput): Promise<job_bids> {
        try {
            if(!id) {
    ...
    public async getJobBidsByJobId(job_id: string): Promise<job_bids[]> {
        try {
            ...
            throw new AppError('Error getting job bids by job id', 500, 'DB_JOB_BIDS_GET_BY_JOB_ID_FAILED');
        }
    }
    ...
    public async getJobBidsByFreelancerId(freelancer_id: string): Promise<job_bids[]> {
        try {
            ...
            throw new AppError('Error getting job bids by freelancer id', 500, 'DB_JOB_BIDS_GET_BY_FREELANCER_ID_FAILED');
        }
    }
